...
Tous les groupes seront répertoriés ici avec la possibilité de modifier, supprimer et créer de nouveaux groupes. Lors de la configuration initiale de MoveToData dans votre établissement, trois groupes seront préconfigurés :

- Administrateurs des Utilisateurs
- Administrateurs de Groupe
- Administrateurs de Projet

Les groupes peuvent aider à gérer les permissions des utilisateurs de MoveToData. Ils peuvent varier de la consultation uniquement à l'édition. Le système de gestion des groupes de MoveToData facilite la délégation des permissions à un groupe de personnes.

## Création d'un Groupe

La création d'un groupe dans MoveToData est un processus simple.
...
Les Membres sont le rôle de base dans un groupe, ils ne peuvent pas modifier les groupes et ne peuvent accéder qu'au groupe de projet auquel ils ont été assignés dans MoveToData.
