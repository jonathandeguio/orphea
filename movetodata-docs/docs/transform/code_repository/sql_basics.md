...
:::info
<code>car_prices</code>: in the above example, car_prices references a DataFrame that represents a Dataset stored within MoveToData and is used a source.

<code>car_prices_out</code>: is the output dataset from <code>source_dataset</code>. Once you trigger a build with your code, the results are saved into a new Dataset file in MoveToData, which you can explore once the build completes.
:::
