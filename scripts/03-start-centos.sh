#!/usr/bin/env bash
# =============================================================================
# Orphea Platform — Script 03 : Démarrage sur CentOS 7 / 8 / 9
# Usage : bash 03-start-centos.sh [--stack core|snap|tycho|all]
#
# Différences vs Ubuntu (03-start.sh) :
#   - Pré-vol SELinux : restorecon + chcon sur /orphea avant docker compose up
...
stop_existing() {
  ...
  if [[ ${found} -eq 1 ]]; then
    ...
    # Essai via docker compose (projet orphea)
...
start_core() {
  ...
  preflight_selinux "${ORPHEA_MOUNT_PATH}"         "/orphea"
  preflight_selinux "${ORPHEA_MOUNT_PATH}/postgres" "/orphea/postgres"
  preflight_selinux "${ORPHEA_MOUNT_PATH}/redis"    "/orphea/redis"
  preflight_selinux "${ORPHEA_MOUNT_PATH}/boson"    "/orphea/boson"
  preflight_selinux "/etc/orphea"                   "/etc/orphea"
  ...
  # Vérifier que /etc/orphea/saml.yml est présent et complet
  # (requis pour que Spring crée le bean RelyingPartyRegistrationRepository)
  if [[ ! -f /etc/orphea/saml.yml ]]; then
    warn "/etc/orphea/saml.yml absent — exécutez d'abord : bash 00-install-system-centos.sh"
  elif ! grep -q "platform-default-login" /etc/orphea/saml.yml; then
    warn "/etc/orphea/saml.yml incomplet (platform-default-login manquant) — correction..."
    sed -i '1s/^/platform-default-login: password\n\n/' /etc/orphea/saml.yml
    success "/etc/orphea/saml.yml corrigé"
  fi
...
http {
    ...
    server {
        ...
        location /api/ {
            proxy_pass http://orphea-boson:8080/api/;
        ...
        location /saml2/ {
            proxy_pass http://orphea-boson:8080/saml2/;
            proxy_set_header Host $host;
            proxy_set_header X-Real-IP $remote_addr;
        }
...
start_snap() {
  ...
  preflight_selinux "${ORPHEA_MOUNT_PATH}/snap"  "/orphea/snap"
...
start_tycho() {
  ...
  # --- Réseau orphea-network (créé par core) ---
  if ! docker network ls --format '{{.Name}}' | grep -q "^orphea-network$"; then
    warn "Réseau orphea-network absent"
  ...
  preflight_selinux "${ORPHEA_MOUNT_PATH}/tycho"    "/orphea/tycho"
  preflight_selinux "${ORPHEA_MOUNT_PATH}/postgres/tycho" "/orphea/postgres/tycho"
  ...
  local init_exit
  init_exit=$(docker inspect --format='{{.State.ExitCode}}' orphea-tycho-init 2>/dev/null || echo "N/A")
  if [[ "${init_exit}" == "0" ]]; then
    success "Initialisation Tycho réussie (exit 0)"
  elif [[ "${init_exit}" != "N/A" ]]; then
    warn "Init Tycho exitcode=${init_exit} — vérifiez : docker logs orphea-tycho-init"
  fi
...
echo ""
echo -e "${CYAN}╔══════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}║   Orphea Platform — Démarrage sur CentOS     ║${NC}"
echo -e "${CYAN}╚══════════════════════════════════════════════╝${NC}"
echo ""
...
if [[ "${SELINUX_STATUS}" == "Enforcing" ]]; then
  ...
  echo "  2. Appliquer : restorecon -Rv /orphea"
