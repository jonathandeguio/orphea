...
const getWaterFallChartOptions = ({
  ...
  //-----------------------
  let totalSum: { [key: string]: any } = {};

  let accumulateSeries: { [key: string]: any } = {};

  data.map(({ data: seriesData, name }: any, index: number) => {
    seriesData.map((val: any) => {
      totalSum[name] = (totalSum[name] ?? 0) + val[1];
      accumulateSeries[val[0]] = (accumulateSeries[val[0]] ?? 0) + val[1];
    });
  });

  if (chartCustomization.showTotalSum) {
    data.map(({ data: seriesData, name }: any, index: number) => {
      seriesData.push(["Total", totalSum[name]]);
    });
  }

  let accumulateSum = 0;
  let accumulateSumSeries: any[] = [];
  chartData.data?.xAxisData.map((xAxisVal: string) => {
    accumulateSumSeries.push([xAxisVal, accumulateSum]);
    accumulateSum += accumulateSeries[xAxisVal];
  });
  ...
  const VerticalAxisChartOptions = {
    ...
    xAxis: {
      type: "category",
      data: chartCustomization.showTotalSum
        ? [...chartData.data.xAxisData, "Total"]
...
export default getWaterFallChartOptions;
