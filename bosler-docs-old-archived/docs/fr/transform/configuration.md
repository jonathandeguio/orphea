...
- Les référentiels de fonctions sont destinés à écrire une logique métier pouvant être exécutée rapidement dans un cadre opérationnel. Ils sont livrés avec une prise en charge intégrée pour accéder aux données de l'ontologie Bosler et incluent la saisie semi-automatique TypeScript basée sur les types de données Ontology.
...
<p style="text-indent: 20px;"><code>jeu de données source</code>: fait référence à un DataFrame qui représente un Dataset stocké dans Bosler.</p>

<p style="text-indent: 20px;"><code>jeu de données cible</code>: dans cette fonction est l'endroit où vous pouvez définir une série de transformations que vous souhaitez voir appliquées à <code>jeu de données source</code>. Une fois que vous avez déclenché une génération avec votre code, les résultats sont enregistrés dans un nouveau fichier Dataset dans Bosler, que vous pouvez explorer une fois la génération terminée.</p></div>
