...
BASE_URL=https://dev.movetodata.io/api
...
if [ ! -d $SCRIPT_DIR/java ]; then
  ...
  echo | openssl s_client -servername NAME -connect dev.movetodata.io:443 |  sed -ne '/-BEGIN CERTIFICATE-/,/-END CERTIFICATE-/p' > $BUILD_PACKAGE/etc/certs/certificate.crt
  keytool -import -trustcacerts -keystore $SCRIPT_DIR/java/lib/security/cacerts --storepass changeit -noprompt -alias movetodata -file $BUILD_PACKAGE/etc/certs/certificate.crt
