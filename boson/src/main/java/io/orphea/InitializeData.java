...
import io.orphea.platform.library.repository.*;
import io.orphea.sharedutils.CommonService;
...
import static io.orphea.sharedutils.storage.FSMinio.MinioConnection;
...
public class InitializeData {
    ...
    // The below is to create Orphea, first it deletes user.home/orphea directory and then adds data and folders - -
    @Bean
            ...
            if (System.getenv("GENERATE_CERTIFICATE") != null) {
                ...
                if (System.getenv("GENERATE_CERTIFICATE").equals("true")) {
                    if (!System.getProperty("os.name").startsWith("Windows")) { // below only works on linux based systems
                        ...
                        String certPath = System.getenv("ORPHEA_MOUNT_PATH") + "/orphea_ca_cert.pem";
...
//                        addX509CertificateToTrustStore(cert, "orphea", bosonStorePath, "changeit", "PKCS12");
...
//                            FileOutputStream fos = new FileOutputStream(System.getenv("ORPHEA_MOUNT_PATH") + "/orphea_ca_cert.pem");
...
//                        socket.close();
                        ...
                        try {
                            addX509CertificateToTrustStoreViaFile(certPath, "orphea", bosonStorePath, "changeit", "PKCS12");
            ...
            File orpheaGitHome = new File(System.getenv("ORPHEA_MOUNT_PATH") + "/git");
            orpheaGitHome.mkdirs();

            File orpheaGitRemoteHome = new File(System.getenv("ORPHEA_MOUNT_PATH") + "/git/remote");
            orpheaGitRemoteHome.mkdirs();

            File orpheaGitClonedHome = new File(System.getenv("ORPHEA_MOUNT_PATH") + "/git/cloned");
            orpheaGitClonedHome.mkdirs();
            ...
            if (System.getenv("BACKING_FS").equals("s3")) { // Create directories if not exists in s3
                ...
                try {
                    ...
                    boolean found =
                            minioClient.bucketExists(BucketExistsArgs.builder().bucket("orphea").build());
                    if (!found) {
                        minioClient.makeBucket(MakeBucketArgs.builder().bucket("orphea").build());
                    }
                ...
                BlobId orpheaPath = BlobId.of(System.getenv("GS_BUCKET"), "orphea/");
                BlobId datasetPath = BlobId.of(System.getenv("GS_BUCKET"), "orphea/dataset/");
                BlobId filePath = BlobId.of(System.getenv("GS_BUCKET"), "orphea/file/");
                BlobId repositoryPath = BlobId.of(System.getenv("GS_BUCKET"), "orphea/repositories/");
                BlobId sparkStreamingPath = BlobId.of(System.getenv("GS_BUCKET"), "orphea/spark-streaming/");

                // Check if the directory exists
                BlobInfo orpheaPathInfo = storage.get(orpheaPath);
                if (orpheaPathInfo == null) {
                    storage.create(BlobInfo.newBuilder(orpheaPath).setContentType("application/x-directory").build(), new byte[0]);
                }
                ...
                Path orpheaPath = new Path("/orphea");
                Path datasetPath = new Path("/orphea/dataset");
                Path filePath = new Path("/orphea/file");
                Path repositoryPath = new Path("/orphea/repositories");
                Path sparkStreamingPath = new Path("/orphea/spark-streaming");

                if (!fs.exists(orpheaPath)) {
                    fs.mkdirs(orpheaPath);
                }
                ...
                String orpheaPath = System.getenv("LOCAL_FS_DIRECTORY");
                ...
                File orpheaPathDirectory = new File(orpheaPath);

                if (!orpheaPathDirectory.exists()) {
                    boolean orpheaResultDir = orpheaPathDirectory.mkdirs();
                }
            ...
            if (!userRepository.existsByUsername("platform-administrator")) {
                userService.saveUser(new User(null, "Platform Administrator", "platform-administrator", defaultPassword, "Platform", "Administrator", "None",
                        "",
                        "platform-administrator@orphea.io", AuthProvider.local, null, null, false, null, false, null, new UserPreferences(), new NotificationPreferences().getId(), null, null, null, null, null));
            }
            ...
            if (!userRepository.existsByUsername(PlatformInternal.toString())) {
                User platformInternal = userService.saveUser(new User(null, "Platform Internal", PlatformInternal.toString(), "b071181ada05d564a60d", "Platform", "Internal", "None",
                        "",
                        "platform-internal@orphea.io", AuthProvider.local, null, null, false, null, false, null, new UserPreferences(), new NotificationPreferences().getId(), null, null, null, null, null));
            }
...
//            }
            ...
            if (!platformConfigRepository.existsByName("platformConfig")) {
                platformConfigRepository.save(new PlatformConfig(null, new Date(), new Date(), null, null, "Orphea", "platformConfig", true, 250000, 0L, 500, true, 2592000L, true, "Europe/Paris", 2, 2, false, false, null, "https://pypi.python.org/simple/", "http://username:pssword@proxy.example.com", null, "[]", "master", false, false, false));
            }
