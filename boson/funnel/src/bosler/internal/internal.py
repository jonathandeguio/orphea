...
from utils.configurations import BUILD_TOKEN, BOSLER_API, LANGUAGE, BUILD_ID, SPARK_APPLICATION_ID, BUILD_TYPE, \
    REPOSITORY_ID, \
...
class Kitab:
    ...
    def end_transaction(self, dataset_id, branch, build_id = None):
        ...
        api_response = Bosler().api("GET", f"/api/kitab/transaction/{dataset_id}/{branch}/end")
...
class Build:
    ...
    def initiate_notebook_build(self, build_id, user_id, branch):
        ...
        api_response = Bosler().api("POST", "/api/build/buildNotebook", payload, "JSON")
    ...
    def pre_transform(self, sources):
        ...
        api_response = Bosler().api("POST", "/api/funnel/preTransform", payload, "JSON")
    ...
    def resolve_target(self, target, sources, filename, lineno):
        ...
        api_response = Bosler().api("POST", "/api/funnel/resolveTarget", payload, "JSON")
    ...
    def post_transform(self, dataset_id, sources, transactionId):
        ...
        api_response = Bosler().api("POST", "/api/funnel/postTransform", payload, "JSON")
        if api_response.status_code == 200:
            return True
        else:
    ...
    def preview_post_transform(self, result, schema, target):
        ...
        api_response = Bosler().api("POST", f"/api/funnel/previewPostTransform", payload, "JSON")
        if api_response.status_code == 200:
            return True
        else:
...
class Logging:
    ...
    def log(self, status, stage, message, build_id = None, debug=None, checkpoint_dataset_id=None, checkpoint_status=None):
        ...
        #     api_response = Bosler().api("POST", f"/api/funnel/{PREVIEW_ID}/log", payload, "JSON")
        # elif BUILD_TYPE == "DEFAULT":
        api_response = Bosler().api("POST", f"/api/build/{build_id}/log", payload, "JSON")
