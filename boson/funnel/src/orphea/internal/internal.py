...
from utils.configurations import BUILD_TOKEN, ORPHEA_API, LANGUAGE, BUILD_ID, SPARK_APPLICATION_ID, BUILD_TYPE, \
    REPOSITORY_ID, \
...
class Build:
    ...
    def initiate_notebook_build(self, build_id, user_id, branch):
        ...
        api_response = Orphea().api("POST", "/api/build/buildNotebook", payload, "JSON")
    ...
    def resolve_target(self, target, sources, filename, lineno):
        ...
        api_response = Orphea().api("POST", "/api/funnel/resolveTarget", payload, "JSON")
    ...
    def preview_post_transform(self, result, schema, target):
        ...
        api_response = Orphea().api("POST", f"/api/funnel/previewPostTransform", payload, "JSON")
        if api_response.status_code == 200:
            return True
        else:
...
class Logging:
    ...
    def log(self, status, stage, message, build_id = None, debug=None, checkpoint_dataset_id=None, checkpoint_status=None):
        ...
        #     api_response = Orphea().api("POST", f"/api/funnel/{PREVIEW_ID}/log", payload, "JSON")
