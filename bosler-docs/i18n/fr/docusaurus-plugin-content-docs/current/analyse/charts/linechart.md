...
## **Line Chart**
> Un **Line Chart** est une représentation graphique qui utilise une série de points de données reliés par une ligne continue pour montrer les tendances au fil du temps ou pour d'autres variables continues. Cette documentation vous guidera à travers les options de configuration disponibles pour créer un Line Chart dans notre application et fournira des instructions sur la manière de le créer efficacement.
