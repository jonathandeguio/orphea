...
## **Scatter Chart**
> Un Scatter Chart est une représentation graphique qui utilise des points pour afficher la relation entre deux variables numériques. Chaque point représente une observation dans l'ensemble de données, ce qui le rend utile pour identifier des corrélations ou des motifs entre les variables. Cette documentation vous guidera à travers les options de configuration disponibles pour créer un scatter chart dans notre application et fournira des instructions sur la manière d'en créer un efficacement.
