...
<code>source_dataset</code>: fait référence à un DataFrame qui représente un Dataset stocké dans Bosler.

<code>target_dataset</code> : dans cette fonction, vous pouvez définir une série de transformations que vous souhaitez voir appliquées à <code>source_dataset</code>. Une fois que vous avez déclenché une génération avec votre code, les résultats sont enregistrés dans un nouveau fichier Dataset dans Bosler, que vous pouvez explorer une fois la génération terminée.
