...
```python
 from bosler.functions import funnel
 import pyspark.sql.functions as F
```

<code>source_dataset</code>: references a DataFrame that represents a Dataset stored within Bosler.

<code>target_dataset</code>: within this function is where you may define a series of transformations you want to see applied to <code>source_dataset</code>. Once you trigger a build with your code, the results are saved into a new Dataset file in Bosler, which you can explore once the build completes.
