...
Pour établir une connexion à MoveToData, une source de données configurée est requise. Il est important de noter qu'une source de données ne peut pas être utilisée directement dans MoveToData, les données doivent être synchronisées dans un ensemble de données avant de pouvoir être utilisées.
