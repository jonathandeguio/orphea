...
<div style="text-align: justify"> Une source de données est tout système de données externe pouvant être connecté à Orphea. Les exemples d'une source de données incluent, mais ne sont pas limités à :
...
Pour établir une connexion à Orphea, une source de données configurée est requise. Il est important de noter qu'une source de données ne peut pas être utilisée directement dans Orphea, les données doivent être synchronisées dans un ensemble de données avant de pouvoir être utilisées.
