...
public class FractalController {
    ...
    ResponseEntity<Object> open(Principal principal,
        ...
        return new ResponseEntity<>(response.okResponse("Repository with repoId " +
                repositoryId + " opened successfully"), HttpStatus.OK);
...
//            new Git(repository).checkout().setName(branch).call();
            ...
            for (Map<String, Object> contents : userInput) {
                ...
                byte[] valueDecoded = Base64.decodeBase64(fileContent);
...
//            }
            ...
            CommitCommand commitCommand = git.commit().setMessage(new Date().toString())
                    .setCommitter(userService.getUser(principal.getName()).getUsername(), userService.getUser(principal.getName()).getUsername());
            commitCommand.call();
...
//        template.convertAndSend("/topic/repository/" + repositoryId + "/" + branch, textMessage);
    ...
    ResponseEntity<Object> startJupyter(Principal principal, HttpServletRequest httpServletRequest) throws Exception {
        ...
        envVars.add(new V1EnvVar().name("MOVETODATA_USERID").value(userId.toString()));
        envVars.add(new V1EnvVar().name("BASE_URL").value(System.getenv("BASE_URL")));
        envVars.add(new V1EnvVar().name("MOVETODATA_API").value("http://boson:8080"));
        envVars.add(new V1EnvVar().name("DB_USERNAME").valueFrom(new V1EnvVarSource().secretKeyRef(new V1SecretKeySelector().name("movetodata-secret").key("boson-db-username"))));
        envVars.add(new V1EnvVar().name("DB_PASSWORD").valueFrom(new V1EnvVarSource().secretKeyRef(new V1SecretKeySelector().name("movetodata-secret").key("boson-db-password"))));
        ...
        envVars.add(new V1EnvVar().name("NOTEBOOK_DIR").value(System.getenv("MOVETODATA_MOUNT_PATH") + "/git/cloned"));
        envVars.add(new V1EnvVar().name("NOTEBOOK_TOKEN").value(token));
        envVars.add(new V1EnvVar().name("NOTEBOOK_TOKEN_TTL").value(tokenTTL));

        envVars.add(new V1EnvVar().name("TOKEN_SECRET").valueFrom(new V1EnvVarSource().secretKeyRef(new V1SecretKeySelector().name("movetodata-secret").key("token-secret"))));
        envVars.add(new V1EnvVar().name("FRACTAL_TEMPLATES_TOKEN").valueFrom(new V1EnvVarSource().secretKeyRef(new V1SecretKeySelector().name("movetodata-secret").key("fractal-templates-token"))));
        envVars.add(new V1EnvVar().name("MINIO_ACCESS_KEY").valueFrom(new V1EnvVarSource().secretKeyRef(new V1SecretKeySelector().name("movetodata-secret").key("minio-username"))));
        envVars.add(new V1EnvVar().name("MINIO_SECRET_KEY").valueFrom(new V1EnvVarSource().secretKeyRef(new V1SecretKeySelector().name("movetodata-secret").key("minio-password"))));
        envVars.add(new V1EnvVar().name("GOOGLE_CLOUD_CREDENTIALS").valueFrom(new V1EnvVarSource().secretKeyRef(new V1SecretKeySelector().name("movetodata-secret").key("google-cloud-credentials"))));
        ...
        volumeMounts.add(new V1VolumeMount().mountPath("/movetodata").name("movetodata-nfs-volume"));
        container.setVolumeMounts(volumeMounts);

        container.setWorkingDir(System.getenv("MOVETODATA_MOUNT_PATH"));
        ...
        pvcVolumeSource.setClaimName("movetodata-pvc");

        V1Volume movetodataNfsVolume = new V1Volume();
        movetodataNfsVolume.setName("movetodata-nfs-volume");
        movetodataNfsVolume.setPersistentVolumeClaim(pvcVolumeSource);

        List<V1Volume> volumes = new ArrayList<>();
        volumes.add(movetodataNfsVolume);
        ...
        annotations.put("beta.cloud.google.com/backend-config", "{\"default\": \"movetodata-backend-config\"}");
        ...
        String ingressName = "movetodata-ingress";
        ...
        ingressRule.setHost(System.getenv("dev.movetodata.io"));  // TODO : Change this to auto
