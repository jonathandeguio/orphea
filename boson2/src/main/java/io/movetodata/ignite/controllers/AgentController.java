...
import io.movetodata.ignite.library.models.*;
import io.movetodata.ignite.library.repository.*;
...
//            System.out.println(build.toString());
    ...
    public static boolean cloneOrPullIgnite(File igniteRepository, String IgniteRepositoryPath) throws GitAPIException, IOException {
        ...
        if (!igniteRepository.exists()) {
            String igniteRepositoryUrl = "https://" + System.getenv("FRACTAL_TEMPLATES_TOKEN") + "@github.com/MoveToData-io/Ignite.git";
...
//        String baseUrl = String.format("%s://%s:%d/api", request.getScheme(), request.getServerName(), request.getServerPort());
        ...
        String igniteBasePath = System.getenv("MOVETODATA_MOUNT_PATH") + "/ignite";
...
//        String baseUrl = String.format("%s://%s:%d/api", request.getScheme(), request.getServerName(), request.getServerPort());
        ...
        String igniteBase = System.getenv("MOVETODATA_MOUNT_PATH") + "/ignite";
...
//            FileUtils.copyDirectory(javaPathExtractedFolder, javaBuildPathFolder);
            ...
            // get movetodata certificate form chrome. click on lock on url then (Mac) Option Key + drag the lock to notepad
            // this fails if cert is not there : keytool -import -trustcacerts -keystore ../Library/Java/JavaVirtualMachines/azul-16.0.2/Contents/Home/lib/security/cacerts --storepass changeit -noprompt -alias movetodata -file ./movetodata_public_cert.cer
            // keytool -import -trustcacerts -keystore /User/rakeshmalik/Library/Java/JavaVirtualMachines/openjdk-16.0.1/Contents/Home/lib/security/cacerts --storepass changeit -noprompt -alias movetodata -file ./movetodata_cert.pem

            // oncloud : keytool -import -trustcacerts -keystore $JAVA_HOME/lib/security/cacerts --storepass changeit -noprompt -alias movetodata -file movetodata_ca_cert.pem
...
//                    String hostname = "dev.movetodata.io";  // TODO : change to auto
...
//                    String serverName = url.getHost();
                    ...
                    addX509CertificateToTrustStore(cert, "movetodata", storePath, "changeit", "PKCS12");
                }
            }
