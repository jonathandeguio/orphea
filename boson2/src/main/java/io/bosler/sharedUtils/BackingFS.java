...
public class BackingFS {
    ...
    public static boolean deleteFilesFromGS(UUID datasetId, String branch) {
        try {
            ...
            String prefix = "bosler/datasets/" + datasetId + "/" + branch + "/";
    ...
    public static boolean deleteFilesFromHDFS(UUID datasetId, String branch) {
        ...
        try (FileSystem fs = FileSystem.get(conf)) {
            Path path = new Path(String.format("/bosler/datasets/%s/%s/", datasetId, branch));
    ...
    public static String getDatasetPath(UUID datasetId, String branch) throws Exception {
        ...
        switch (backingFs) {
            case "s3":
                datasetPath = String.format("s3a://bosler/datasets/%s/%s", datasetId, branch);
                break;
            case "gs":
                datasetPath = String.format("gs://%s/bosler/datasets/%s/%s", System.getenv("GS_BUCKET"), datasetId, branch);
                break;
            case "hdfs":
                datasetPath = String.format("%s/bosler/datasets/%s/%s", System.getenv("HDFS_ENDPOINT"), datasetId, branch);
                break;
            default:
                throw new Exception("Unknown backing FS: " + backingFs);
        }
    ...
    public static List<Map<String, Object>> getListOfFiles(UUID datasetId, String branch) throws Exception {
        ...
        switch (backingFS) {
            ...
            case "gs":
                ...
                Storage storage = StorageOptions.newBuilder().setCredentials(credentials).build().getService();
                String directory = String.format("bosler/datasets/%s/%s/", datasetId, branch);
