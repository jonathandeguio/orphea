package io.orphea.bob.controllers;

import io.orphea.bob.library.models.*;
...
import static io.orphea.sharedUtils.Redis.deleteCacheWithWildCard;
...
public class BuildController {
    ...
    public ResponseEntity<Object> abortBuild(Principal principal,
        ...
        List<V1Pod> podList =
                coreApi
                        .listNamespacedPod("orphea", "false", null, null,
        ...
        if (pod != null) {
            deletePod("orphea", pod.getMetadata().getName());
        }
...
//        template.convertAndSend("/topic/build/" + buil + "/" + branch, socketMessage);
    ...
    public ResponseEntity<Object> log(Principal principal,
        ...
        /*
//        kubectl -n orphea logs orphea-spark-buildId-driver
...
//            logOptions.setFollow(true); // set to true to follow the logs as they are written
            ...
            String log = coreApi.readNamespacedPodLog(pod.getMetadata().getName(), "orphea", null, true, null,
