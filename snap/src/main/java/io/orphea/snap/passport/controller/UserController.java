...
import io.orphea.snap.passport.enums.UserData;
import io.orphea.snap.passport.exception.ResourceNotFoundException;
import io.orphea.snap.passport.library.Auth;
import io.orphea.snap.passport.library.models.*;
import io.orphea.snap.passport.library.repository.*;
import io.orphea.snap.passport.library.service.AuthzService;
import io.orphea.snap.passport.library.service.GroupService;
import io.orphea.snap.passport.library.service.UserService;
import io.orphea.snap.passport.security.AuthUser;
import io.orphea.snap.passport.security.CurrentUser;
import io.orphea.snap.platform.library.services.PlatformConfigService;
import io.orphea.snap.sharedutils.Response.ErrorDTO;
...
public class UserController {
    ...
    public ResponseEntity<List<UserDTO>> getUsersAll() throws IOException {
        ...
        for (User user : userService.getUsers()) {
            if (!user.getUsername().equals("platform-internal")) {
...
//    }
    ...
    @Operation(summary = "To disable MFA")
    @PostMapping("/disableMfa/{userId}")
    public ResponseEntity<Object> disableMfa(Principal principal, @PathVariable("userId") UUID requestedUserId) {
        UUID userId = userService.getUser(principal.getName()).getId();
        User authenticateUser = userService.getUser(principal.getName());
        ...
        if (!userId.equals(requestedUserId) && !authzService.isPlatformAdmin(userId)
                && !authzService.isUserAdmin(userId))
            return new ResponseEntity<>("Access Denied to update password of the user", HttpStatus.FORBIDDEN);

        User user = userRepository.getReferenceById(requestedUserId);
        user.setMfaAttributes(null);
        user.setIsMfaEnabled(false);
        userRepository.save(user);

        return new ResponseEntity<>("Disabled MFA Successfully", HttpStatus.OK);
    }
    ...
    public ResponseEntity<Object> updateMetaData(Principal principal, @RequestBody UpdateMetadataRequest updateMetadataRequest) {
        ...
        if (userRepository.existsById(updateMetadataRequest.getId())) {
            ...
            if (!Objects.equals(user.getProfileImage(), updateMetadataRequest.getProfileImage())) {
                user.setProfileImage(updateMetadataRequest.getProfileImage());
            }
