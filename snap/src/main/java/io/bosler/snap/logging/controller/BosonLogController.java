...
import io.bosler.snap.logging.library.models.ApplicationLogModel;
...
public class BosonLogController {
    ...
    @PostMapping(path = "/applicationLogs/{deploymentId}/{metricName}")
    public ResponseEntity<Object> createCSVForApplicationLogs(Principal principal,
        ...
        return loggingService.createCSV(principal, applicationLogModels, deploymentId, "/application", "Application.csv", ApplicationLogModel.class, "boson", metricName);
    }

    @PostMapping("/accessLogs/{deploymentId}/{metricName}")
    @Operation(summary = "Create accessLog CSV")
    public ResponseEntity<Object> createCSVForAccessMetricsLogs(Principal principal,
                                                                @RequestBody List<AccessLogModel> accessLogModels,
                                                                @PathVariable("deploymentId") UUID deploymentId, @PathVariable("metricName") String metricName) {
        return loggingService.createCSV(principal, accessLogModels, deploymentId, "/accessLog", "AccessLog.csv", AccessLogModel.class, "boson", metricName);
    }
}
