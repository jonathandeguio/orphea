...
from bosler import Build
from bosler.functions._checks import Check
from bosler.functions._dataset import Source, Target
from bosler.functions._param import Param
from bosler.functions._utils import _get_most_similar_to
from typing import (
    TYPE_CHECKING,
    Any,
    Callable,
...
class Transform(object):
    ...
    def preTransform(self):
        # print("Preforming pre transform")
        sources_path_or_id = [source.alias for source in self.sources.values()]
        ...
        for source in self.sources.values():
            ...
            dataset_id = response["sources"][_index]
            source.update_properties_after_pre_transform(dataset_id, transaction_id, source_schema, encoding, branch_type)
            _index += 1

        _targetIndex = 0
        for target in self.targets.values():
            dataset_id = response["target"][_targetIndex]
            transaction_id = response["transactionId"][_targetIndex]
            target.update_properties_after_pre_transform(dataset_id, transaction_id)

            _targetIndex += 1

    def postTransform(self):
        print("Preforming post transform")
