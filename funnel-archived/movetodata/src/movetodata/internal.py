...
MOVETODATA_API = os.environ.get('MOVETODATA_API')
...
class Kitab:
    ...
    def start_transaction(self, dataset_id, branch):
        api_response = MoveToData().api("GET", f"/api/kitab/transaction/{dataset_id}/{branch}/start?buildId={BUILD_ID}")
...
class Build:
    ...
    def preview_result(self, result, schema):
        ...
        api_response = MoveToData().api("POST", f"/api/funnel/previewResult", payload, "JSON")
        if api_response.status_code == 200:
            return True
        else:
...
class Logging:
    ...
    def log(self, status, stage, message, debug=None):
        ...
        if BUILD_TYPE == "PREVIEW" and status == Constants().ERROR:
            api_response = MoveToData().api("POST", f"/api/funnel/log", payload, "JSON")
