# db nodes setup
resource "vcd_vapp_vm" "worker_node" {
  ...
  # Attach Data Disk
  disk {
    name        = vcd_independent_disk.worker_disk[count.index].name
    bus_number  = 1
    unit_number = 0
  }
  ...
  network {
    ...
    ip                 = var.worker_static_ip[count.index]
    is_primary         = true
  }

  # Second Network Interface (with DHCP) for NFS
  network {
    type               = var.vcd_org_network_type
  ...
  guest_properties = {
    "guest.hostname"        = "worker-${format("%02d", count.index + 1)}"
    "guestinfo.ip_address"  = var.worker_static_ip[count.index]
