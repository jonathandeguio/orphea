...
LOCK_FILE = "/tmp/bosler_check_updates.lock"
...
def load_image():
    tar_file_path = os.path.join(BOSLER_IMAGE_DIR, f"{IMAGE}.tar")
    if not os.path.isfile(tar_file_path):
        print(f"{datetime.now()} : {RED}[ERROR]{RESET} : Tar file '{tar_file_path}' not found!")
        exit(1)

    os.chdir(BOSLER_IMAGE_DIR)
    try:
...
def download_image():
    ...
    DOWNLOAD_IMAGE = IMAGE.replace("boslerDocs", "bosler-docs").replace("sparkHistoryServer", "spark-history-server")
    image_path = os.path.join(BOSLER_IMAGE_DIR, f"{IMAGE}.tar")
...
def helm_upgrade():
    os.chdir(HELM_DIR)
    subprocess.run(f"helm upgrade bosler charts/{HELM_CHART} -f charts/{HELM_CHART}/{HELM_VALUES}", shell=True)
...
if __name__ == "__main__":
    ...
    BOSLER_IMAGE_DIR = os.path.join(BUNDLE_DIR, "images/bosler")
