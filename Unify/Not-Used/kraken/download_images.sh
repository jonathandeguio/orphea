...
cp env.sh update_movetodata.sh bundle/$TAG
...
echo ""
echo "Note : Copy to the installation server : (e.g. gcloud compute scp "update_movetodata_$TAG.tar.gz" <server>:~/ ) "
echo ""
echo "Login to server"
echo ""
echo "Then Run : sudo tar -C /movetodata -xf update_movetodata_$TAG.tar.gz && sudo /movetodata/bundle/$TAG/update_movetodata.sh"
echo ""
