...
if [ -f /tmp/google-storage-sa-key-file.json.base64 ];
  ...
  cat configurations/movetodata-secrets.yaml | sed -e "s/google-storage-sa/$CLOUD_SA_KEY/g"| kubectl apply -f -
else
...
# kubectl apply -f configurations/movetodata-callisto.yaml
cat configurations/movetodata-callisto.yaml |sed -e "s/movetodata-334213/$PROJECT_ID/g"|sed -e "s/datasets_collections_septbos22/$BUCKET_NAME/g"|sed -e "s/10.90.240.2/$DATABASE_HOST/g"|kubectl apply -f - 

kubectl apply -f configurations/boson-minio-storage.yaml 

kubectl apply -f configurations/boson-db-deployment.yaml

# Don't need below since db is moved out of kubernetes
...
cat configurations/julia-deployment.yaml |sed -e "s/movetodata-334213/$PROJECT_ID/g"|kubectl apply -f -
...
cat configurations/boson-app-deployment.yaml |sed -e "s/movetodata-334213/$PROJECT_ID/g"|sed -e "s/datasets_collections_septbos22/$BUCKET_NAME/g"|sed -e "s/10.90.240.2/$DATABASE_HOST/g"|kubectl apply -f -
...
cat configurations/frontend-deployment.yaml |sed -e "s/movetodata-334213/$PROJECT_ID/g"|kubectl apply -f -
cat configurations/parler-deployment.yaml |sed -e "s/movetodata-334213/$PROJECT_ID/g"|kubectl apply -f -
...
cat configurations/movetodata-spark-history-server.yaml |sed -e "s/movetodata-334213/$PROJECT_ID/g"|kubectl apply -f -
...
echo "|                                                                          |"
echo "|                                                                          |"
echo "|   Connect to MoveToData  : https://dev.movetodata.io                             |"
echo -e "|   Connect to Swagger : https://dev.movetodata.io/swagger-ui.html          |"
echo "|                                                                          |"
echo "+--------------------------------------------------------------------------+"
echo ""
