...
ARTIFACTORY_PATH="europe-west1-docker.pkg.dev/$PROJECT_ID/orphea-cr"
...
bundle() {
    ...
    if [[ $prune == [yY] || $prune == [yY][eE][sS] ]]; then
        ...
        if [ -f orphea_bundle.tar.gz ]; then
            rm orphea_bundle.tar.gz
        fi
    ...
    # scripts
    cp build_orphea_offline.sh bundle/
    cp orphea.repo bundle/configs
    ...
    cp -r onPremiseInstall/orphea_spark_env.sh bundle/deployments

    # create tar bundle
    if [ ! -f orphea_bundle.tar.gz ]; then
        tar cfz orphea_bundle.tar.gz bundle
    fi
    ...
    echo ""
    echo "Note : Copy to the installation server and run :  sudo mkdir /orphea && sudo tar -C /orphea -xf orphea_bundle.tar.gz && sudo /orphea/bundle/build_orphea_offline.sh prepare"
    echo ""
...
setup_local_repo()
{
    ...
    sudo cd /orphea/bundle/packages

    sudo cd $BUNDLE_DIR/packages
    sudo tar -C $BUNDLE_DIR -xf $BUNDLE_DIR/packages/packages.tar.gz

    # install create repo
    sudo yum -y -q --disablerepo=* localinstall /orphea/bundle/packages/deltarpm-3.6-3.el7.x86_64.rpm
    sudo yum -y -q --disablerepo=* localinstall /orphea/bundle/packages/python-deltarpm-3.6-3.el7.x86_64.rpm
    sudo yum -y -q --disablerepo=* localinstall /orphea/bundle/packages/libxml2-python-2.9.1-6.el7_9.6.x86_64.rpm
    sudo yum -y -q --disablerepo=* localinstall /orphea/bundle/packages/createrepo-0.9.9-28.el7.noarch.rpm

    sudo createrepo /orphea/bundle/packages > /dev/null

    sudo cp $BUNDLE_DIR/configs/orphea.repo /etc/yum.repos.d
...
setup_container()
{
    ...
    then
            ...
            if [ $OFFLINE_INSTALL = "true" ]; then
                if [ -f /etc/yum.repos.d/orphea.repo ]; then
                    yum install -y -q --disablerepo=* --enablerepo=orphea install docker-ce
                else
                    echo "ERROR : orphea repo file not found in /etc/yum/repos.d"
                fi
            else
                if [ -f /etc/yum.repos.d/orphea.repo ]; then
                    yum install -y -q --disablerepo=orphea docker-ce
                else
...
EOF
            ...
            if [ $OFFLINE_INSTALL = "true" ]; then
                if [ -f /etc/yum.repos.d/orphea.repo ]; then                    
                    yum install -y -q --disablerepo=* --enablerepo=orphea kubelet kubeadm kubectl --disableexcludes=kubernetes
                else
                    echo "ERROR : orphea repo file not found in /etc/yum/repos.d"
                fi
            else
                if [ -f /etc/yum.repos.d/orphea.repo ]; then
                    yum install -y -q --disablerepo=orphea kubelet kubeadm kubectl --disableexcludes=kubernetes
                else
...
install()
{
    # if [ ! -f /tmp/orphea_install_prepared ]; then
    ...
    # clean up
    [ -f /tmp/orphea_install_prepared ] && rm /tmp/orphea_install_prepared
}
...
setup_orphea()
{
    ...
    #image tags
    sudo docker tag $ARTIFACTORY_PATH/frontend                europe-west1-docker.pkg.dev/orphea-bnp/orphea-cr/frontend
    sudo docker tag $ARTIFACTORY_PATH/boson                   europe-west1-docker.pkg.dev/orphea-bnp/orphea-cr/boson
    sudo docker tag $ARTIFACTORY_PATH/julia                   europe-west1-docker.pkg.dev/orphea-bnp/orphea-cr/julia
    sudo docker tag $ARTIFACTORY_PATH/parler                  europe-west1-docker.pkg.dev/orphea-bnp/orphea-cr/parler
    sudo docker tag $ARTIFACTORY_PATH/funnel                  europe-west1-docker.pkg.dev/orphea-bnp/orphea-cr/funnel
    sudo docker tag $ARTIFACTORY_PATH/spark-history-server    europe-west1-docker.pkg.dev/orphea-bnp/orphea-cr/spark-history-server
    ...
    sudo kubectl create -f $BUNDLE_DIR/deployments/configurations/orphea-namespace.yaml
    sudo kubectl apply -f $BUNDLE_DIR/deployments/configurations/orphea-secrets.yaml
    # sudo kubectl label namespace orphea istio-injection=enabled
    ...
    while true
    do
    sudo kubectl -n orphea get pods |grep boson-db|grep Running > /dev/null
    if [ $? == 0 ]; then
        echo "|"
        sleep 5
        # kubectl exec -it boson-db-5dd84575cf-z6l2j -- bin/bash -c "su - postgres -c \"psql -c 'DROP DATABASE boson;'\""
        sudo kubectl -n orphea exec -it `kubectl -n orphea get pods |awk '{print $1}'|grep boson-db` -- /bin/bash -c "su - postgres -c \"psql -c 'CREATE DATABASE boson;'\""
        sudo kubectl -n orphea exec -it `kubectl -n orphea get pods |awk '{print $1}'|grep boson-db` -- /bin/bash -c "su - postgres -c \"psql -c 'CREATE DATABASE kepler;'\""
        break
    fi
    echo -n "."
    sleep 5
    ...
    sudo kubectl -n orphea exec -it `kubectl -n orphea get pods |awk '{print $1}'|grep minio` -- /bin/bash -c "mkdir -p /storage/datasets"
    sudo kubectl -n orphea exec -it `kubectl -n orphea get pods |awk '{print $1}'|grep minio` -- /bin/bash -c "mkdir -p /storage/spark-streaming/checkpoint"
    ...
    while true
    do
    sudo kubectl -n orphea get pods |grep -v boson-db|grep boson|grep Running > /dev/null
    if [ $? == 0 ]; then
        echo "|"
        break
    fi
    ...
    sudo kubectl apply -f $BUNDLE_DIR/deployments/configurations/orphea-spark-history-server.yaml

    sudo kubectl apply -f $BUNDLE_DIR/deployments/configurations/orphea-ingress.yaml


    # setup spark
    sudo helm install -f $BUNDLE_DIR/deployments/spark/spark-helm-values.yaml orphea $BUNDLE_DIR/deployments/spark/spark-operator-1.1.15.tgz --namespace orphea
...
prepare()
{
    ...
    sudo echo "Step1" >/tmp/orphea_install_prepared
}
