...
bundle() {
    ...
    echo ""
    echo "Note : Copy to the installation server and run :  sudo mkdir /bosler && sudo tar -C /bosler -xf bosler_bundle.tar.gz && sudo /bosler/bundle/build_bosler_offline.sh prepare"
    echo ""
    ...
    then
            ...
            if [ $OFFLINE_INSTALL = "true" ]; then
                if [ -f /etc/yum.repos.d/bosler.repo ]; then
                    yum install -y -q --disablerepo=* --enablerepo=bosler install docker-ce
                else
                    echo "ERROR : bosler repo file not found in /etc/yum/repos.d"
                fi
            else
                if [ -f /etc/yum.repos.d/bosler.repo ]; then
                    yum install -y -q --disablerepo=bosler docker-ce
                else
...
install()
{
    # if [ ! -f /tmp/bosler_install_prepared ]; then
    ...
    # clean up
    [ -f /tmp/bosler_install_prepared ] && rm /tmp/bosler_install_prepared
}
...
setup_bosler()
{
    ...
    #image tags
    sudo docker tag $ARTIFACTORY_PATH/frontend                europe-west1-docker.pkg.dev/bosler-bnp/bosler-cr/frontend
    sudo docker tag $ARTIFACTORY_PATH/boson                   europe-west1-docker.pkg.dev/bosler-bnp/bosler-cr/boson
    sudo docker tag $ARTIFACTORY_PATH/julia                   europe-west1-docker.pkg.dev/bosler-bnp/bosler-cr/julia
    sudo docker tag $ARTIFACTORY_PATH/parler                  europe-west1-docker.pkg.dev/bosler-bnp/bosler-cr/parler
    sudo docker tag $ARTIFACTORY_PATH/funnel                  europe-west1-docker.pkg.dev/bosler-bnp/bosler-cr/funnel
    sudo docker tag $ARTIFACTORY_PATH/spark-history-server    europe-west1-docker.pkg.dev/bosler-bnp/bosler-cr/spark-history-server
    ...
    sudo kubectl create -f $BUNDLE_DIR/deployments/configurations/bosler-namespace.yaml
    sudo kubectl apply -f $BUNDLE_DIR/deployments/configurations/bosler-secrets.yaml
    # sudo kubectl label namespace bosler istio-injection=enabled
    ...
    while true
    do
    sudo kubectl -n bosler get pods |grep boson-db|grep Running > /dev/null
    if [ $? == 0 ]; then
        echo "|"
        sleep 5
        # kubectl exec -it boson-db-5dd84575cf-z6l2j -- bin/bash -c "su - postgres -c \"psql -c 'DROP DATABASE boson;'\""
        sudo kubectl -n bosler exec -it `kubectl -n bosler get pods |awk '{print $1}'|grep boson-db` -- /bin/bash -c "su - postgres -c \"psql -c 'CREATE DATABASE boson;'\""
        sudo kubectl -n bosler exec -it `kubectl -n bosler get pods |awk '{print $1}'|grep boson-db` -- /bin/bash -c "su - postgres -c \"psql -c 'CREATE DATABASE kepler;'\""
        break
    fi
    echo -n "."
    sleep 5
    ...
    sudo kubectl -n bosler exec -it `kubectl -n bosler get pods |awk '{print $1}'|grep minio` -- /bin/bash -c "mkdir -p /storage/datasets"
    sudo kubectl -n bosler exec -it `kubectl -n bosler get pods |awk '{print $1}'|grep minio` -- /bin/bash -c "mkdir -p /storage/spark-streaming/checkpoint"
    ...
    while true
    do
    sudo kubectl -n bosler get pods |grep -v boson-db|grep boson|grep Running > /dev/null
    if [ $? == 0 ]; then
        echo "|"
        break
    fi
    ...
    sudo kubectl apply -f $BUNDLE_DIR/deployments/configurations/bosler-spark-history-server.yaml

    sudo kubectl apply -f $BUNDLE_DIR/deployments/configurations/bosler-ingress.yaml


    # setup spark
    sudo helm install -f $BUNDLE_DIR/deployments/spark/spark-helm-values.yaml bosler $BUNDLE_DIR/deployments/spark/spark-operator-1.1.15.tgz --namespace bosler
...
prepare()
{
    ...
    sudo echo "Step1" >/tmp/bosler_install_prepared
}
