...
export VERSION_orphea_docs=0.0.3
...
bundle() {
    ...
    if [ $DOWNLOAD_IMAGES = "true" ]; then
        ...
        declare -a images=(
            ...
            "$ARTIFACTORY_PATH/orphea-docs:$VERSION_orphea_docs"
        ...
        for image_tag in "${images[@]}"; do
            ...
            # if [ ! -f bundle/images/orphea/$image_name.tar ] || [ ! -f bundle/images/kubernetes/$image_name.tar ]; then
            if [ ! -f "bundle/images/orphea/$image_name.tar" ] && [ ! -f "bundle/images/kubernetes/$image_name.tar" ]; then
                ...
                if [[ $image_tag == *"$ARTIFACTORY_PATH"* ]]; then
                    ...
                    new_tag=$(echo $image_tag | sed 's,'"$ARTIFACTORY_PATH"',orphea.io,g')

                    docker tag $image_tag $new_tag
                    docker save --output bundle/images/orphea/$image_name.tar $new_tag >/dev/null
                else
    ...
    if [ $BUNDLE_OS = "CentOS" ]; then
        cp OS/CentOS/orphea.repo bundle/configs
    ...
    echo ""
    echo "Note : Copy to the installation server : (e.g. gcloud compute scp orphea_bundle.tar.gz orphea-instance-with-access:~/ ) "
    echo ""
    echo "Login to server : gcloud beta compute ssh --zone "$COMPUTE_ZONE" "orphea-instance-with-access"  --tunnel-through-iap --project "$PROJECT_ID""
    echo ""
    echo "Then Run :  sudo mkdir /orphea && sudo tar -C /orphea -xf orphea_bundle.tar.gz && sudo /orphea/bundle/build_orphea_offline.sh prepare"
    echo ""
...
install() {
    ...
    if [[ "$confirm" == [yY] || "$confirm" == [yY][eE][sS] ]]; then
        # if [ ! -f /var/tmp/orphea_install_prepared ]; then
        ...
        if [ $MYOS = "Ubuntu" ]; then
            ...
            # export BUNDLE_DIR=/orphea/bundle
        ...
        # clean up
        [ -f /var/tmp/orphea_install_prepared ] && rm /var/tmp/orphea_install_prepared
    fi
}
...
setup_orphea() {
    ...
    if [[ "$confirm" == [yY] || "$confirm" == [yY][eE][sS] ]]; then
        helm install orphea charts/orphea-gke -f charts/orphea-gke/$VALUES

        # helm upgrade orphea charts/orphea-gke -f charts/$HELM_CHART/$HELM_VALUES
    fi
    ...
    # if [[ "$confirm" == [yY] || "$confirm" == [yY][eE][sS] ]]; then
    #     helm install orphea-hdfs charts/hdfs-charts
    # fi
    ...
    # if [[ "$confirm" == [yY] || "$confirm" == [yY][eE][sS] ]]; then
    #     helm install orphea-nfs charts/nfs-server
    # fi

    cd - >/dev/null

    # setup spark
    helm install -f $BUNDLE_DIR/deployments/spark/spark-helm-values.yaml orphea $BUNDLE_DIR/deployments/spark/spark-operator-1.1.27.tgz --namespace $NAMESPACE --set webhook.enable=true
    ...
    if [[ "$confirm" == [yY] || "$confirm" == [yY][eE][sS] ]]; then
        cd $BUNDLE_DIR/deployments/configurations/helm
        helm upgrade orphea charts/orphea-gke -f charts/orphea-gke/$VALUES
...
prepare() {
    ...
    sudo echo "Step1" >/var/tmp/orphea_install_prepared
}

usage() {
    echo "$0 bundle|prepare|ubuntuInstall|CentOSprepare|CentOSinstall|setup_orphea"
}
