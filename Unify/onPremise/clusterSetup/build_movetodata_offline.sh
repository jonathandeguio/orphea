...
export VERSION_movetodata_docs=0.0.3
...
bundle() {
    ...
    if [ $DOWNLOAD_IMAGES = "true" ]; then
        ...
        if [ ! -d bundle/images ]; then
            ...
            mkdir -p bundle/images/movetodata
        fi
        ...
        declare -a images=(
            ...
            "$ARTIFACTORY_PATH/movetodata-docs:$VERSION_movetodata_docs"
        ...
        for image_tag in "${images[@]}"; do
            ...
            # if [ ! -f bundle/images/movetodata/$image_name.tar ] || [ ! -f bundle/images/kubernetes/$image_name.tar ]; then
            if [ ! -f "bundle/images/movetodata/$image_name.tar" ] && [ ! -f "bundle/images/kubernetes/$image_name.tar" ]; then
                ...
                if [[ $image_tag == *"$ARTIFACTORY_PATH"* ]]; then
                    ...
                    new_tag=$(echo $image_tag | sed 's,'"$ARTIFACTORY_PATH"',movetodata.io,g')

                    docker tag $image_tag $new_tag
                    docker save --output bundle/images/movetodata/$image_name.tar $new_tag >/dev/null
                else
    ...
    if [ $BUNDLE_OS = "CentOS" ]; then
        cp OS/CentOS/movetodata.repo bundle/configs
    ...
    echo ""
    echo "Note : Copy to the installation server : (e.g. gcloud compute scp movetodata_bundle.tar.gz movetodata-instance-with-access:~/ ) "
    echo ""
    echo "Login to server : gcloud beta compute ssh --zone "$COMPUTE_ZONE" "movetodata-instance-with-access"  --tunnel-through-iap --project "$PROJECT_ID""
    echo ""
    echo "Then Run :  sudo mkdir /movetodata && sudo tar -C /movetodata -xf movetodata_bundle.tar.gz && sudo /movetodata/bundle/build_movetodata_offline.sh prepare"
    echo ""
...
install() {
    ...
    if [[ "$confirm" == [yY] || "$confirm" == [yY][eE][sS] ]]; then
        # if [ ! -f /var/tmp/movetodata_install_prepared ]; then
        ...
        if [ $MYOS = "Ubuntu" ]; then
            ...
            # export BUNDLE_DIR=/movetodata/bundle
        ...
        # clean up
        [ -f /var/tmp/movetodata_install_prepared ] && rm /var/tmp/movetodata_install_prepared
    fi
}
...
setup_movetodata() {
    ...
    if [[ "$confirm" == [yY] || "$confirm" == [yY][eE][sS] ]]; then
        helm install movetodata charts/movetodata-gke -f charts/movetodata-gke/$VALUES
    ...
    # setup spark
    helm install -f $BUNDLE_DIR/deployments/spark/spark-helm-values.yaml movetodata $BUNDLE_DIR/deployments/spark/spark-operator-1.1.27.tgz --namespace $NAMESPACE --set webhook.enable=true
    ...
    if [[ "$confirm" == [yY] || "$confirm" == [yY][eE][sS] ]]; then
        cd $BUNDLE_DIR/deployments/configurations/helm
        helm upgrade movetodata charts/movetodata-gke -f charts/movetodata-gke/$VALUES
...
prepare() {
    ...
    sudo echo "Step1" >/var/tmp/movetodata_install_prepared
}

usage() {
    echo "$0 bundle|prepare|ubuntuInstall|CentOSprepare|CentOSinstall|setup_movetodata"
}
