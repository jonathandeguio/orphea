...
BOSLER_IMAGE_DIR="$BUNDLE_DIR/images/bosler"
...
function build_image() {
    if ctr -n k8s.io images ls -q | grep -q "$IMAGE" | grep -q "$IMAGE_VERSION"; then
        ...
        cd /bosler/repos/"${IMAGE}"
        git pull
        docker build . --tag bosler.io/"${IMAGE}":"${IMAGE_VERSION}"

        if [ ! -f "${BOSLER_IMAGE_DIR}/${IMAGE}-${IMAGE_VERSION}.tar" ]; then
            docker save --output "${BOSLER_IMAGE_DIR}/${IMAGE}-${IMAGE_VERSION}.tar" bosler.io/"${IMAGE}":"${IMAGE_VERSION}"
        fi
    fi
}
...
function clean_up {
    ...
    if [[ $prune == [yY] || $prune == [yY][eE][sS] ]]; then
        if [ -d "${BOSLER_IMAGE_DIR}" ]; then
            rm -rf "${BOSLER_IMAGE_DIR}/*"
        fi
...
# push to azure
function azure_push() {
    docker tag "bosler.io/${IMAGE}:${IMAGE_VERSION}" "boslerdevacr.azurecr.io/${IMAGE}:${IMAGE_VERSION}"

    docker push "boslerdevacr.azurecr.io/${IMAGE}:${IMAGE_VERSION}"
}

# Upgrade Helm
function helm_upgrade() {
    cd "$HELM_DIR" || exit
    helm upgrade bosler charts/"$HELM_CHART" -f "charts/${HELM_CHART}/${HELM_VALUES}"
}
