...
const Loader = ({ computedSize, color, content, size, type }: TLoaderProps) => {
  return (
    ...
    /> */}
      ...
      {/* <div className="login-icon-orphea">

    </div> */}
    </div>
  );
};
const OrpheaLoader: FC<TLoaderProps> = ({
  content,
  size = "medium",
  color = "var(--orphea-border-color-default)",
  type = "atom",
}: TLoaderProps) => {
      ...
      <div className="fallbackWrapper">
        <div
          ...
          style={{
            ...
            border: `${borderSize} dotted var(--orphea-border-color-default)`,
          }}
        ></div>
      </div>
    );
