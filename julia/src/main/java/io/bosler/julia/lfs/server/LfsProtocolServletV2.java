package io.bosler.julia.lfs.server;
...
public abstract class LfsProtocolServletV2 extends LfsProtocolServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res)
		...
		System.out.println(req.getHeader(HDR_AUTHORIZATION));
		log.info("request header : " + HDR_AUTHORIZATION);

		res.setContentType(CONTENTTYPE_VND_GIT_LFS_JSON);
		LargeFileRepository repo = null;
		try {
			repo = getLargeFileRepository(request, path,
					req.getHeader(HDR_AUTHORIZATION), req.getRemoteUser() != null);
			if (repo == null) {
