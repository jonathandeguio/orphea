package io.orphea.controller;

import com.fasterxml.jackson.databind.JsonNode;
import com.fasterxml.jackson.databind.ObjectMapper;
import io.orphea.BobEnums.BuildStage;
import io.orphea.BobEnums.BuildStatus;
import io.orphea.utils.BosonApiCalls;
import io.orphea.utils.SparkUtils;
...
import static io.orphea.utils.BackingFS.deleteDatasetFiles;
import static io.orphea.utils.BackingFS.getDatasetPath;
...
public class SqlTransformController {
    ...
    public void transform(UUID repositoryId, String branch, String scriptPath) throws Exception {
        ...
        File file = new File(System.getenv("ORPHEA_MOUNT_PATH") + "/tmp/OrpheaTransformTemporary/" + repositoryId);
...
//        }
        ...
        reader = new BufferedReader(new InputStreamReader(new FileInputStream(System.getenv("ORPHEA_MOUNT_PATH") + "/tmp/OrpheaTransformTemporary/" + repositoryId + "/" + scriptPath), StandardCharsets.UTF_8));
...
//            BosonApiCalls.BosonLog(BuildStatus.INFO, BuildStage.RUNNING, "Starting Transform on " + targetDatasetLogicalPath, null);
            ...
            // temp files
            deleteDatasetFiles(UUID.fromString(targetDatasetId), "/OrpheaTransformTemporary/");
...
//                System.out.println("Deleting temporary files : " + getDatasetPath(UUID.fromString(targetDatasetId)) + "/OrpheaTransformTemporary/");

                FileUtils.deleteDirectory(new File(getDatasetPath(UUID.fromString(targetDatasetId)) + "/OrpheaTransformTemporary/"));
            } catch (Exception e) {
...
//                System.out.println("Deleting temp files 2");
                FileUtils.deleteDirectory(new File(getDatasetPath(UUID.fromString(targetDatasetId)) + "/OrpheaTransformTemporary/"));
                FileUtils.deleteDirectory(new File(System.getenv("ORPHEA_MOUNT_PATH") + "/tmp/OrpheaTransformTemporary/" + System.getenv("REPOSITORY_ID")));
            } catch (Exception e) {
                System.out.println("deleting files " + e);
            }
    ...
    private static String replaceTargetDataset(String line, String targetDatasetLogicalPath, String targetView, String targetDatasetId) throws Exception {
        return line.replace("`" + targetDatasetLogicalPath + "`",
                "`" + targetView + "`" + "\n USING parquet \n OPTIONS ('path' '" +
                        getDatasetPath(UUID.fromString(targetDatasetId)) + "/OrpheaTransformTemporary/" +
