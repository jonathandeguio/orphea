...
import static io.orphea.utils.OrpheaMinio.MinioConnection;
import static io.orphea.utils.Utils.isBase64;
...
public class BackingFS {
    ...
    public static boolean deleteFilesFromGS(UUID datasetId, String branch) {
        try {
            ...
            String prefix = "orphea/dataset/" + datasetId + "/" + branch + "/";
    ...
    public static boolean deleteFilesFromHDFS(UUID datasetId, String branch) {
        ...
        try (FileSystem fs = FileSystem.get(conf)) {
            Path path = new Path(String.format("/orphea/dataset/%s/%s/", datasetId, branch));
    ...
    public static String getDatasetPath(UUID datasetId) throws Exception {
        ...
        switch (backingFs) {
            case "s3":
                datasetPath = String.format("s3a://orphea/dataset/%s", datasetId);
                break;
            case "gs":
                datasetPath = String.format("gs://%s/orphea/dataset/%s", System.getenv("GS_BUCKET"), datasetId);
                break;
            case "hdfs":
                datasetPath = String.format("%s/orphea/dataset/%s", System.getenv("HDFS_ENDPOINT"), datasetId);
                break;
            case "localfs":
