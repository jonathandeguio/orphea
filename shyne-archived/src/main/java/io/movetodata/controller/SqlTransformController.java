...
import io.movetodata.utils.BosonApiCalls;
...
import static io.movetodata.utils.BackingFS.deleteDatasetFiles;
import static io.movetodata.utils.BackingFS.getDatasetPath;
...
public class SqlTransformController {
    ...
    public void transform(UUID repositoryId, String branch, String scriptPath) throws Exception {
        ...
        File file = new File(System.getenv("MOVETODATA_MOUNT_PATH") + "/tmp/MoveToDataTransformTemporary/" + repositoryId);
...
//        }
        ...
        reader = new BufferedReader(new InputStreamReader(new FileInputStream(System.getenv("MOVETODATA_MOUNT_PATH") + "/tmp/MoveToDataTransformTemporary/" + repositoryId + "/" + scriptPath), StandardCharsets.UTF_8));
...
//            BosonApiCalls.BosonLog(BuildStatus.INFO, BuildStage.RUNNING, "Starting Transform on " + targetDatasetLogicalPath, null);
            ...
            // temp files
            deleteDatasetFiles(UUID.fromString(targetDatasetId), "/MoveToDataTransformTemporary/");
...
//                System.out.println("Deleting temporary files : " + getDatasetPath(UUID.fromString(targetDatasetId)) + "/MoveToDataTransformTemporary/");

                FileUtils.deleteDirectory(new File(getDatasetPath(UUID.fromString(targetDatasetId)) + "/MoveToDataTransformTemporary/"));
            } catch (Exception e) {
...
//                System.out.println("Deleting temp files 2");
                FileUtils.deleteDirectory(new File(getDatasetPath(UUID.fromString(targetDatasetId)) + "/MoveToDataTransformTemporary/"));
                FileUtils.deleteDirectory(new File(System.getenv("MOVETODATA_MOUNT_PATH") + "/tmp/MoveToDataTransformTemporary/" + System.getenv("REPOSITORY_ID")));
            } catch (Exception e) {
                System.out.println("deleting files " + e);
            }
    ...
    private static String replaceTargetDataset(String line, String targetDatasetLogicalPath, String targetView, String targetDatasetId) throws Exception {
        return line.replace("`" + targetDatasetLogicalPath + "`",
                "`" + targetView + "`" + "\n USING parquet \n OPTIONS ('path' '" +
                        getDatasetPath(UUID.fromString(targetDatasetId)) + "/MoveToDataTransformTemporary/" +
