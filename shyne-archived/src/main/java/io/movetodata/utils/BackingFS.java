...
import static io.movetodata.utils.MoveToDataMinio.MinioConnection;
...
public class BackingFS {
    ...
    public static boolean deleteFilesFromGS(UUID datasetId, String branch) {
        try {
            ...
            String prefix = "movetodata/dataset/" + datasetId + "/" + branch + "/";
    ...
    public static boolean deleteFilesFromHDFS(UUID datasetId, String branch) {
        ...
        try (FileSystem fs = FileSystem.get(conf)) {
            Path path = new Path(String.format("/movetodata/dataset/%s/%s/", datasetId, branch));
    ...
    public static String getDatasetPath(UUID datasetId) throws Exception {
        ...
        switch (backingFs) {
            case "s3":
                datasetPath = String.format("s3a://movetodata/dataset/%s", datasetId);
                break;
            case "gs":
                datasetPath = String.format("gs://%s/movetodata/dataset/%s", System.getenv("GS_BUCKET"), datasetId);
                break;
            case "hdfs":
                datasetPath = String.format("%s/movetodata/dataset/%s", System.getenv("HDFS_ENDPOINT"), datasetId);
                break;
            case "localfs":
